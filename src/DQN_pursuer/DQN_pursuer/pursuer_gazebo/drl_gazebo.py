...
from ..pursuer_environment.drl_environment import ARENA_LENGTH, ARENA_WIDTH, ENABLE_DYNAMIC_GOALS
from ..common.settings import ENABLE_TRUE_RANDOM_GOALS, ENABLE_GENERATE_GOALS
...
class DRLGazebo(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.entity_dir_path = (os.path.dirname(os.path.realpath(__file__))).replace(
            'DQN_pursuer/lib/python3.8/site-packages/DQN_pursuer/pursuer_gazebo',
        ...
        ************************************************************"""
        ...
        self.goal_pose_pub = self.create_publisher(Pose, 'new_goal_pose', QoSProfile(depth=10))
        ...
        # Initialise servers
        self.task_succeed_server    = self.create_service(RingGoal, 'task_succeed_p', self.task_succeed_callback)
        self.task_fail_server       = self.create_service(RingGoal, 'task_fail_p', self.task_fail_callback)

        self.obstacle_coordinates   = self.get_obstacle_coordinates()
        # self.timer = self.create_timer(0.1, self.update_goal_pose)
    ...
    *******************************************************************************"""
    ...
    def odom_callback(self, msg):
        # Extract pose from the odometry message
        new_goal_pose = Pose()
        new_goal_pose.position.x = msg.pose.pose.position.x
        new_goal_pose.position.y = msg.pose.pose.position.y
        new_goal_pose.orientation = msg.pose.pose.orientation

        # Set and publish the new goal pose
        self.goal_x = new_goal_pose.position.x
        self.goal_y = new_goal_pose.position.y
        self.goal_pose_pub.publish(new_goal_pose)
        print(f"New goal set from odometry: ({self.goal_x}, {self.goal_y})")

    def update_goal_pose(self):
        # Example of moving the goal linearly along the x-axis
        speed = 0.1  # meters per second
        self.goal_x += speed  # Increment x position
        if self.goal_x > ARENA_LENGTH / 2:  # Example condition to reverse direction or reset position
            self.goal_x = -ARENA_LENGTH / 2

        # Check if the new goal position is valid
        if not self.goal_is_valid(self.goal_x, self.goal_y):
            self.goal_x = 0.0  # Reset to center or some other handling logic

        # Once the goal position is updated, publish it
        self.publish_callback()
    ...
    def get_obstacle_coordinates(self):
        ...
        for wall in root.find('model').findall('link'):
            ...
            self.o_pose_x = float(pose[0])
            self.o_pose_y = float(pose[1])
            # self.goal_x = self.o_pose_x
            # self.goal_y = self.o_pose_y
            if rotation == 0:
            ...
            point_1 = [self.o_pose_x + size_x / 2, self.o_pose_y + size_y / 2]
