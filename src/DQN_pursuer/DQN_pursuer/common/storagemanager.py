...
class StorageManager:
    def __init__(self, name, load_session, load_episode, device, stage):
        if load_session and name not in load_session:
            ...
            while True:
                pass
        self.machine_dir = (os.getenv('DRLNAV_BASE_PATH') + '/src/DQN_pursuer/model/' + str(socket.gethostname()))
        if 'examples' in load_session:
            self.machine_dir = (os.getenv('DRLNAV_BASE_PATH') + '/src/DQN_pursuer/model/')
        self.name = name
